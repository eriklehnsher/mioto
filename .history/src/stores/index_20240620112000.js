...
const store = createStore({
  ...
  actions: {
    async loginUser({ commit }, credentials) {
      try {
        const response = await axios.post(`${baseURL}/user/login`, credentials); // Gọi API đăng nhập
    ...
    async registerUser({ commit }, userData) {
      try {
        const response = await axios.post(`${baseURL}/user/register`, userData); // Gọi API đăng ký
