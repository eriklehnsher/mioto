...
const baseURL = process.env.VUE_APP_BACKEND_URL || "http://localhost:8080"; // Điều chỉnh baseURL cho phù hợp
...
const store = createStore({
  ...
  actions: {
    ...
    async registerUser({ commit }, userData) {
      try {
        const response = await axios.post(`${baseURL}/register`, userData); // Gọi API đăng ký
    ...
    // Các actions khác như loadUser, logoutUser
  },
  getters: {
